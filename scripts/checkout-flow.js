...
class ActorVaultCheckoutFlow {
  ...
  static ownerId(document) {
    ...
    return game.users
      .filter(user => !user.isGM && document.testUserPermission?.(
        user,
  ...
  static async importStored(packActorId) {
    ...
    if (!owner || owner.isGM) throw new Error("The stored actor's player owner no longer exists.");

    const data = stored.toObject();
    delete data._id;
    data.folder = this.restoreFolder(record, owner)?.id || null;
    data.ownership = this.ownership(owner.id);
    foundry.utils.setProperty(data, AVC_RECORD_PATH, { ...record, checkedOutAt: Date.now(), updatedAt: Date.now() });

    let actor;
    try {
      [actor] = await Actor.implementation.createDocuments([data], { keepId: false });
    } catch (error) {
      throw new Error(`Foundry denied the actor import. The player role needs permission to create Actors. ${error.message}`);
    }
  ...
  static async removeActorTokens(actorId) {
    for (const scene of game.scenes) {
      const tokenIds = [...scene.tokens].filter(token => token.actorLink && token.actorId === actorId).map(token => token.id);
      if (tokenIds.length) await scene.deleteEmbeddedDocuments("Token", tokenIds);
    }
    for (const combat of game.combats) {
      const combatantIds = [...combat.combatants].filter(combatant => combatant.actorId === actorId).map(combatant => combatant.id);
  ...
  static async ensureVaultFolders(pack, owner) {
    ...
    if (!root) root = await Folder.create({ name: AVC_PLAYERS_FOLDER, type: "Actor", folder: null }, { pack: pack.collection });
  ...
  static async exportOne(actor, pack) {
    ...
    const oldEntry = index.find(entry => foundry.utils.getProperty(entry, AVC_RECORD_PATH)?.vaultId === record.vaultId);
    ...
    let replacement;
    [replacement] = await Actor.implementation.createDocuments([data], { pack: pack.collection, keepId: false });
  ...
  static enhance(app, element) {
    ...
    for (const button of root.querySelectorAll('button[data-action="activate"]')) {
      ...
      const vaultId = indexEntry ? foundry.utils.getProperty(indexEntry, AVC_RECORD_PATH)?.vaultId : null;
...
Hooks.on("renderApplicationV2", (app, element) => {
  if (app?.id === "actor-vault-app") queueMicrotask(() => ActorVaultCheckoutFlow.enhance(app, element));
});
